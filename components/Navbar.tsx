...
import { Menu, X, Command, Search, Sparkles, LayoutDashboard, Database, FileText, Image as ImageIcon, Code, UserCheck, LogOut } from "lucide-react";
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {user ? (
              <div className="flex items-center space-x-2">
                ...
                <Link
                  href="/dashboard"
                  className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900 text-xs font-semibold"
                >
                ...
                <button
                  onClick={logout}
                  className="p-1.5 rounded-lg border border-slate-200 dark:border-slate-800 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 transition-all"
                  title="Sign Out"
                >
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg text-xs font-bold border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900 transition-all"
                >
                  Sign In
                </button>
                <button
                  onClick={openAuthModal}
                  className="px-3.5 py-1.5 rounded-lg text-xs font-extrabold bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white shadow-lg shadow-indigo-500/20 hover:opacity-90 transition-all flex items-center space-x-1"
                >
          ...
          <div className="px-2 pt-2 pb-4 space-y-1 sm:px-3">
            ...
            {user ? (
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between px-3">
                <span className="text-sm font-semibold text-emerald-500">{user.name} (Pro)</span>
                <button onClick={logout} className="text-xs font-bold text-red-500 hover:underline">Sign Out</button>
              </div>
            ) : (
              <button
                onClick={() => { setIsOpen(false); openAuthModal(); }}
                className="w-full mt-2 px-3 py-2 rounded-md text-center text-sm font-bold bg-gradient-to-r from-indigo-600 to-purple-600 text-white"
              >
